...
void draw_texel(
	...
	// Another solution here for segmentation fault error
	// This will cap the index, keeping it in its limits
	// Index numbers such 5000 does not cause any crash but very large numbers like >10k causes segfault. I think here difference is larger numbers that we try to use are not available to user, used by some program. That's why it causes access violation. DANGER ZONE.
	int texIndex = ((texture_width * tex_y) + tex_x)%(texture_width * texture_height); //4096
...
void draw_textured_triangle(
	...
	uint32_t* texture)
{
	// TODO: Loop through all the pixels of our triangle to render them based on the color from the texture

	// We need to sort the vertices by y-coordinate ascending (y0 < y1 < y2)
	// We do also sort the UV positions
	if (y0 > y1)
	{
		int_swap(&y0, &y1);
		int_swap(&x0, &x1);
	...
	if (y1 - y0 != 0) {
		...
		for (int y = y0; y <= y1; y++)
		{
			...
			for (int x = x_start; x < x_end; x++)
			{
				...
				//draw_pixel(x, y, x % 5 ? 0xffffff00 : 0xffffffff);
			}
		}
	}
